...
#
# Tags look like `v<ClickHouse version>-<release>`:
#
# - if defaults/main.yml points at a ClickHouse version that has never been
#   released, the release counter restarts at 0 (`v26.7.4.59-0`)
# - otherwise the counter is incremented (`v26.7.4.59-1`), but only if something
...
version="$(sed -nE 's|^clickhouse_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the ClickHouse version from $defaults_path"
	exit 1
fi
